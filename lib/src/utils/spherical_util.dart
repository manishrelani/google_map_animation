...
class SphericalUtil {
  ...
  static LatLng computeOffset(LatLng from, num distance, num heading) {
    ...
    return LatLng(
      MathUtil.toDegrees(asin(sinLat)).toDouble(),
  ...
  static LatLng? computeOffsetOrigin(LatLng to, num distance, num heading) {
    ...
    return LatLng(
      MathUtil.toDegrees(fromLatRadians).toDouble(),
  ...
  static LatLng interpolate(LatLng from, LatLng to, num fraction) {
    ...
    return LatLng(
      MathUtil.toDegrees(lat).toDouble(),
      MathUtil.toDegrees(lng).toDouble(),
    );
  }
  ...
  static double angleLerp(double from, double to, double t) {
    var shortestAngle = ((to - from) + 180) % 360 - 180;
    ...
    if (result < 1e-6 && result > -1e-6) return to;
